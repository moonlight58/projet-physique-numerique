...
def create_high_quality_animation(
    output_dir, size=100, density=0.65, p=0.5, max_steps=300
):
    """
...
def main():
    ...
    parser.add_argument(
        "-s", "--size", type=int, default=80, help="Taille de la grille"
    )
    parser.add_argument(
        "-d", "--density", type=float, default=0.65, help="Densité d'arbres"
    )
    parser.add_argument(
        "-p",
        "--probability",
    ...
    parser.add_argument(
        "--max-steps", type=int, default=150, help="Nombre maximum de timesteps"
    )
    ...
    if args.animation_type in ["all", "neighborhood"]:
        create_neighborhood_comparison(
            output_dir, args.size, args.density, args.probability
        )
